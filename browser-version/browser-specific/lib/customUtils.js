...
function byteArrayToBase64(uint8) {
  ...
  for (let i = 0; i < length; i += 3) {
    const temp = (uint8[i] << 16) + (uint8[i + 1] << 8) + (uint8[i + 2]);
 ...
 * (if the base64 conversion yields too many pluses and slashes) but that's not an issue here
